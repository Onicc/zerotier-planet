  ...
  return (
    <Card bordered={false} className="section-card">
      ...
      <Table rowKey={(member) => memberId(member)} dataSource={members} scroll={{ x: 1000 }} pagination={{ pageSize: 20, hideOnSinglePage: true }} locale={{ emptyText: t('networks.noMembers') }}
        columns={[
          { title: t('common.name'), dataIndex: 'name', width: 210, render: (value: string, member) => <Input defaultValue={value} placeholder={t('networks.friendlyName')} onPressEnter={(event) => patch.mutate({ id: memberId(member), body: { name: event.currentTarget.value.trim() } })} onBlur={(event) => { if (event.target.value.trim() !== (value || '')) patch.mutate({ id: memberId(member), body: { name: event.target.value.trim() } }); }} /> },
          { title: t('networks.memberId'), width: 150, render: (_, member) => <code>{memberId(member)}</code> },
          { title: t('common.status'), width: 130, filters: undefined, render: (_, member) => <StatusBadge dot tone={member.peerState === 'online' || member.peerState === 'controller' ? 'success' : member.peerState === 'relay' ? 'warning' : member.peerState === 'error' ? 'error' : 'default'} label={t(`common.${member.peerState === 'controller' ? 'controller' : member.peerState || 'offline'}`)} /> },
          ...
          { title: t('networks.bridge'), width: 90, render: (_, member) => <Switch size="small" checked={Boolean(member.activeBridge)} onChange={(activeBridge) => patch.mutate({ id: memberId(member), body: { activeBridge } })} /> },
          { title: t('networks.ipAssignments'), minWidth: 300, render: (_, member) => <Space size={[4, 4]} wrap>{member.ipAssignments?.map((ip, index) => <Tag key={`${ip}-${index}`} closable onClose={(event) => { event.preventDefault(); removeIp.mutate({ id: memberId(member), index }); }}>{ip}</Tag>)}<Button size="small" type="dashed" icon={<PlusOutlined />} onClick={() => setIpMember(member)}>{t('common.add')}</Button></Space> },
          { title: '', width: 55, fixed: 'right', render: (_, member) => <Tooltip title={t('common.remove')}><Button type="text" danger icon={<DeleteOutlined />} onClick={() => confirmRemove(member)} /></Tooltip> },
        ]} />
