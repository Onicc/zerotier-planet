...
export function GuidePage() {
  ...
  return <div className="page-stack"><PageHeader title={t('page.guideTitle')} subtitle={t('page.guideSubtitle')} eyebrow={t('common.guide')} />
    ...
    <Card bordered={false} className="guide-card guide-troubleshoot-card"><span className="guide-icon"><CheckCircleOutlined /></span><div className="guide-card-copy"><h2>{t('guide.troubleshoot')}</h2><p>{t('guide.troubleshootCopy')}</p><ul className="troubleshooting-checklist"><li>9994/TCP & UDP</li><li><code>zerotier-cli peers</code></li><li><code>zerotier-cli listnetworks</code></li><li>Planet → Network ID → Authorization → Routes → Pools</li></ul><Button type="primary" ghost onClick={() => navigate('/overview')}>{t('common.details')}</Button></div></Card>
  </div>;
}
