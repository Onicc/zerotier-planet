...
type CommandKind = 'planet' | 'linux' | 'macos';

export function DeliveryPage() {
  const { t, i18n } = useTranslation();
  const { message } = App.useApp();
  const overview = useOverviewQuery();
  const controller = useControllerQuery();
  const [step, setStep] = useState(0);
  const [networkId, setNetworkId] = useState('');
  const [includeNetwork, setIncludeNetwork] = useState(false);
  const [ttl, setTtl] = useState(600);
  const [generating, setGenerating] = useState(false);
  const [commands, setCommands] = useState<Record<string, GeneratedCommand>>({});
  const resetCommands = () => setCommands({});
  ...
  const buildCommand = async (kind: CommandKind): Promise<[CommandKind, GeneratedCommand]> => {
    ...
    return [kind, { value, expiresAt: expiryTime(link.expiresIn) }];
  };
  ...
  const generateAll = async () => {
    ...
    resetCommands();
    try {
      const generated = await Promise.all((['planet', 'linux', 'macos'] as CommandKind[]).map(buildCommand));
      setCommands(Object.fromEntries(generated));
      setStep(2);
  ...
  const changeStep = async (next: number) => {
    if (next === 2 && Object.keys(commands).length !== 3) {
      await generateAll();
      return;
    }
    setStep(next);
  ...
  return <div className="page-stack"><PageHeader title={t('page.deliveryTitle')} subtitle={t('page.deliverySubtitle')} eyebrow={t('common.delivery')} />
    <Card bordered={false} className="delivery-workflow"><Steps current={step} onChange={(next) => void changeStep(next)} responsive items={[{ title: t('delivery.stepNetwork') }, { title: t('delivery.stepPolicy') }, { title: t('delivery.stepGenerate') }]} />
      <div className="delivery-step-content">
        {step === 0 && <div className="delivery-policy-panel"><SafetyCertificateOutlined /><div><h2>{t('delivery.targetTitle')}</h2><p>{t('delivery.targetCopy')}</p><label>{t('delivery.network')}</label><Select allowClear value={networkId || undefined} onChange={(value) => { setNetworkId(value || ''); if (!value) setIncludeNetwork(false); resetCommands(); }} placeholder={t('delivery.interactive')} options={networks.map((network) => ({ value: network.nwid, label: `${network.name || network.nwid} · ${network.nwid}` }))} /><div className={`delivery-network-option${networkId ? '' : ' disabled'}`}><Checkbox checked={includeNetwork} disabled={!networkId} onChange={(event) => { setIncludeNetwork(event.target.checked); resetCommands(); }}>{t('delivery.includeNetwork')}</Checkbox></div></div></div>}
        {step === 1 && <div className="delivery-policy-panel"><CheckCircleOutlined /><div><h2>{t('delivery.policyTitle')}</h2><p>{t('delivery.policyCopy')}</p><label>{t('delivery.ttl')}</label><Select value={ttl} onChange={(value) => { setTtl(value); resetCommands(); }} options={[{ value: 600, label: t('delivery.ten') }, { value: 1800, label: t('delivery.thirty') }, { value: 3600, label: t('delivery.hour') }]} /></div></div>}
        {step === 2 && <><h2>{t('delivery.commandTitle')}</h2><Alert type="info" showIcon message={selected ? `${selected.name || selected.nwid} · ${includeNetwork ? t('delivery.includeNetwork') : t('delivery.interactive')}` : t('delivery.interactive')} />
          <Row gutter={[16, 16]} className="command-grid">{cards.map((card) => <Col xs={24} xl={8} key={card.key}><Card bordered={false} className="command-card"><div className="command-card-icon">{card.icon}</div><h3>{card.title}</h3><p>{card.copy}</p><CodeBlock code={commands[card.key].value} /><small className="expiry-label">{t('delivery.expires', { time: commands[card.key].expiresAt.toLocaleTimeString(i18n.language) })}</small></Card></Col>)}</Row></>}
      </div>
      <div className="delivery-navigation"><Button disabled={step === 0 || generating} onClick={() => setStep((value) => value - 1)}>{t('common.back')}</Button>{step < 2 && <Button type="primary" loading={generating} onClick={() => void changeStep(step + 1)}>{t('common.next')}</Button>}</div>
    </Card>
  </div>;
}
