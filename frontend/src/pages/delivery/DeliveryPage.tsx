  ...
  return <div className="page-stack"><PageHeader title={t('page.deliveryTitle')} subtitle={t('page.deliverySubtitle')} eyebrow={t('common.delivery')} />
    <Card bordered={false} className="delivery-workflow"><Steps current={step} onChange={(next) => void changeStep(next)} responsive items={[{ title: t('delivery.stepNetwork') }, { title: t('delivery.stepPolicy') }, { title: t('delivery.stepGenerate') }]} />
      <div className="delivery-step-content">
        {step === 0 && <div className="delivery-policy-panel"><SafetyCertificateOutlined /><div><h2>{t('delivery.targetTitle')}</h2><p>{t('delivery.targetCopy')}</p><label>{t('delivery.network')}</label><Select allowClear value={networkId || undefined} onChange={(value) => { setNetworkId(value || ''); if (!value) setIncludeNetwork(false); resetCommands(); }} placeholder={t('delivery.interactive')} options={networks.map((network) => ({ value: network.nwid, label: `${network.name || network.nwid} · ${network.nwid}` }))} /><Checkbox className="delivery-network-option" checked={includeNetwork} disabled={!networkId} onChange={(event) => { setIncludeNetwork(event.target.checked); resetCommands(); }}>{t('delivery.includeNetwork')}</Checkbox></div></div>}
